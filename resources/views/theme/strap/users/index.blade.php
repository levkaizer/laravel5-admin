		...
		@foreach($users as $u)
			<tr id="user-{{ $u->id }}">
				...
				<td width="31%" class="text-center">
					<a class="btn btn-primary btn-edit" data-id="{{ $u->id }}" href="/admin/users/edit/{{ $u->id }}">Edit</a>
					<a class="btn btn-info btn-chgpwd" data-id="{{ $u->id }}" data-toggle="modal" data-target="#modal" href="#">Change password</a>
					<a class="btn btn-danger btn-delete" data-id="{{ $u->id }}" href="/admin/users/delete/{{ $u->id }}">Delete</a>
				</td>
			</tr>
		@endforeach
		</tbody>
...
$(function(){
	$('a.btn-delete').on('click', function(){
		if(confirm('Are you sure you want to delete this user?')) {
			...
			$.post('/admin/users/delete/'+uid, postData).done(function(data){
				$('tr#user-'+data.id).fadeOut('fast', function(){
					$(this).remove();
				});
			});
		}
